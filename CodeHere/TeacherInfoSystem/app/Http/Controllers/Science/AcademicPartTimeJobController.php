...
class AcademicPartTimeJobController extends Controller
{
    ...
    public function getVerifiedIndex(){//获取已审核的多个论文信息
        ...
        if ($account->science_level){//如果是超级用户，可以看所有表中的信息
            $academicPartTimeJobs = AcademicPartTimeJob::join('accounts','accounts.userid','=','academicPartTimeJobs.userid')->select('academicPartTimeJobs.id','academicPartTimeJobs.userid','duty','accounts.name','verify_level','accounts.icon_path')->where('verify_level','=',1)->orderBy('academicPartTimeJobs.updated_at','desc')->paginate(6);
        }
    ...
    public function getNotVerifiedIndex(){//获取未审核的多个论文信息
        ...
        if ($account->science_level){//如果是超级用户，可以看所有表中的信息
            $academicPartTimeJobs = AcademicPartTimeJob::join('accounts','accounts.userid','=','academicPartTimeJobs.userid')->select('academicPartTimeJobs.id','academicPartTimeJobs.userid','duty','accounts.name','verify_level','accounts.icon_path')->where('verify_level','=',0)->orderBy('academicPartTimeJobs.updated_at','desc')->paginate(6);
        }
        else{//如果是普通用户，只能看自己的信息
